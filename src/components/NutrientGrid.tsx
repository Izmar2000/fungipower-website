...
const NutrientGrid = ({ dict }: { dict: any }) => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 md:gap-x-12 lg:gap-x-16 text-left">
            <div>
                <h4 className="text-orange-500 font-black text-lg uppercase tracking-[0.2em] mb-8 border-b border-white/10 pb-4 font-outfit">{content.macro}</h4>
                <ul className="space-y-4">
                    {macro.map(n => (
                        <li key={n.symbol} className="text-white text-lg md:text-xl font-medium tracking-tight">
                            <span className="text-orange-400 mr-3 opacity-50">•</span>{n.name} <span className="text-white/20 ml-2 font-light italic">({n.symbol})</span>
                        </li>
                    ))}
                </ul>
            </div>
            <div>
                <h4 className="text-orange-500 font-black text-lg uppercase tracking-[0.2em] mb-8 border-b border-white/10 pb-4 font-outfit">{content.secondary}</h4>
                <ul className="space-y-4">
                    {secondary.map(n => (
                        <li key={n.symbol} className="text-white text-lg md:text-xl font-medium tracking-tight">
                            <span className="text-orange-400 mr-3 opacity-50">•</span>{n.name} <span className="text-white/20 ml-2 font-light italic">({n.symbol})</span>
            ...
            <div>
                <h4 className="text-orange-500 font-black text-lg uppercase tracking-[0.2em] mb-8 border-b border-white/10 pb-4 font-outfit">{content.micro}</h4>
