...
function SampleButton({ label, className }: { label: string, className: string }) {
    const { openSampleModal } = useModals()
    return <button onClick={openSampleModal} className={className}>{label}</button>
}
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-3 mb-8 md:hidden">
                                {[
                                    ...
                                    { num: <CountUp from={0} to={17} prefix="+" suffix="%" delay={0.5} />, label: t.sections?.['s-rapport']?.cards?.[3]?.label || 'IJzer in jong plantsap', sub: t.sections?.['s-rapport']?.cards?.[3]?.sub || 'Fe · sterkere celactiviteit', color: 'rgba(132,204,22,0.08)', border: 'rgba(132,204,22,0.2)', accent: '#84cc16' },
                                    { num: <CountUp from={0} to={46} prefix="+" suffix="%" delay={0.6} />, label: t.sections?.['s-rapport']?.cards?.[4]?.label || 'Zink in oud plantsap', sub: t.sections?.['s-rapport']?.cards?.[4]?.sub || 'Zn · stabiele accumulatie', color: 'rgba(132,204,22,0.08)', border: 'rgba(132,204,22,0.2)', accent: '#84cc16' },
