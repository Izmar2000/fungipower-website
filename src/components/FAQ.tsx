...
const FAQ: React.FC<FAQProps> = ({ hideTitle = false, dict }) => {
  ...
  return (
    ...
    >
      ...
      {!hideTitle && (
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full max-w-7xl bg-orange-500/5 blur-[120px] rounded-full pointer-events-none"></div>
      )}
      ...
      <div className={`max-w-7xl mx-auto px-6 relative z-10 ${hideTitle ? '' : ''}`}>
        {!hideTitle && (
          <h2 className="text-4xl md:text-5xl font-bold text-center text-white mb-24 tracking-tight uppercase font-outfit">
            {content.title}<span className="text-orange-500">{content.titleAccent}</span>
          </h2>
        )}
...
const FAQItem: React.FC<{ faq: { q: string; a: string }; isOpen: boolean; onToggle: () => void }> = ({ faq, isOpen, onToggle }) => {
  return (
    <div className={`rounded-2xl overflow-hidden transition-all duration-300 border ${isOpen
      ? 'bg-[#052e24] border-orange-500/30'
      : 'bg-orange-950/20 border-white/5 hover:border-white/10 hover:bg-orange-900/30'
      }`}>
      <button
        className="w-full px-6 py-5 flex items-center justify-between text-left group"
        onClick={onToggle}
      >
        <span className={`font-bold text-[15px] transition-colors uppercase tracking-wide pr-4 ${isOpen ? 'text-orange-500' : 'text-white group-hover:text-orange-50'}`}>
          {faq.q}
        </span>
        <ChevronDown className={`w-5 h-5 text-orange-500 transform transition-transform duration-300 flex-shrink-0 ${isOpen ? 'rotate-180' : ''}`} />
      </button>
      <div className={`transition-all duration-500 ease-in-out overflow-hidden ${isOpen ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-6 pb-6 text-orange-100/70 leading-relaxed text-sm font-medium border-t border-white/5 pt-4">
          {faq.a}
        </div>
      </div>
    </div>
